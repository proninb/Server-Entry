...
// Encodes one Unicode code point as UTF-8 and appends it to the JSON buffer.
// Surrogate values and code points outside the Unicode range are rejected.
[[nodiscard]] inline bool json_append_utf8(
    json_buffer& output,
    std::uint32_t code_point) noexcept {

    if (code_point <= 0x7f) {
        return output.append(static_cast<char>(code_point));
    }
...
// Validates that the complete byte sequence is well-formed UTF-8.
// Overlong encodings, surrogate code points, invalid continuation bytes, and
// values beyond the Unicode range are rejected.
