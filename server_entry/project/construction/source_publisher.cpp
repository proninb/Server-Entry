...
status publish_source_facts(
    ...
    try {
        // These vectors are reusable scratch projections for one declaration at a
        // time. Builder consumes their spans synchronously before the next clear().
        ...
        if (!opened.ok()) {
            ...
            return
                opened.code ==
                        status_code::initialization_failed
                    ? infrastructure(batch.source)
                    : abort(opened);
        }

        for (const auto& fact : batch.enums) {
            // Named enums require canonical_name; anonymous enums must not carry it.
            if (fact.anonymous ==
            ...
            const auto source_values =
                batch.context->enumerators(fact);
        ...
        for (const auto& fact : batch.aggregates) {
            ...
            const auto source_members =
                batch.context->members(fact);

            if (source_members.size() !=
                fact.member_count) {
                return malformed(batch.source);
            }
            ...
            for (const auto& member : source_members) {
                ...
                if (!member.builtin) {
                    ...
                    // Parser must already have resolved this spelling according to
                    // source-language lookup rules. Publisher only interns it.
                    result =
                        transaction.strings().intern(
                            type_bytes,
                            user_type_name);
