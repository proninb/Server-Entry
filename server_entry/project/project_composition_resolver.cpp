...
namespace
{
struct cached_project { project_configuration configuration; bool loaded = false; };
...
bool try_emit(diagnostic_buffer& buffer, const diagnostic_descriptor& descriptor,
              operation_id operation, std::uint32_t offset = 0) noexcept
{
    try
    {
        buffer.emit({descriptor.id, descriptor.default_severity, operation,
                     {{}, offset, 0}, {}});
        return true;
    }
    ...
    project_composition_statistics* statistics) noexcept
{
    ...
    scoped_timer total_timer{metrics, metric_id::project_composition_resolve_duration};
    try
    {
        std::error_code path_error;
        const auto root_path = normalized_absolute(root_configuration_path, path_error);
        if (path_error)
        {
            try_emit(diagnostics, diagnostics::project_composition_failed, operation);
        ...
        measured.worker_limit = (std::min)(std::size_t{8},
        ...
        // failed. outstanding is exactly queued + active unresolved jobs.
        const auto schedule_locked = [&](const std::filesystem::path& path)
        {
            if (cache.find(path) != cache.end()) { ++cache_hits; return; }
            auto node = std::make_shared<cached_project>();
            const auto [position, inserted] = cache.emplace(path, std::move(node));
            if (!inserted) return;
            try { queue.push_back(path); }
            catch (const std::bad_alloc&) { cache.erase(position); throw; }
            catch (const std::length_error&) { cache.erase(position); throw; }
            ++outstanding;
        };

        for (const auto& item : root_configuration.project)
            if (item.role == project_item_role::project) schedule_locked(item.path);
        stop = outstanding == 0;
        ...
        const auto worker_body = [&]
        {
            while (true)
            {
                ...
                node_ptr node;
                {
                    ...
                    ready.wait(lock, [&]
                    { return internal_failure.load() || stop || !queue.empty(); });
                    if (internal_failure.load() || queue.empty()) return;
                    path = std::move(queue.front());
                    queue.pop_front();
                    node = cache.find(path)->second;
                    ++active_workers;
                    measured.max_active_workers =
                        (std::max)(measured.max_active_workers, active_workers);
                }

                project_configuration loaded;
                diagnostic_buffer local_diagnostics;
                const auto result = load_project_configuration_file(
                    path, operation, local_diagnostics, metrics, loaded);

                // The single normal completion point: each dequeued job
                // decrements outstanding exactly once.
                std::unique_lock lock{mutex};
                --active_workers;
                ...
                if (!result.ok())
                {
                    if (!first_failure)
                    {
                        ...
                        {
                            const auto& record = local_diagnostics.records()[0];
                ...
                if (--outstanding == 0) stop = true;
                ready.notify_all();
            }
        };

        // No exception escapes a thread entry. An exceptional abort wakes every
        // waiter and supersedes normal outstanding accounting.
        const auto worker = [&]
        {
            try { worker_body(); }
            catch (const std::bad_alloc&) { internal_failure.store(true); ready.notify_all(); }
            catch (const std::length_error&) { internal_failure.store(true); ready.notify_all(); }
            catch (const std::system_error&) { internal_failure.store(true); ready.notify_all(); }
        };
        ...
        if (outstanding != 0)
        {
            ...
            for (std::size_t index = 0; index < measured.worker_limit; ++index)
                workers.emplace_back(worker);
            for (auto& thread : workers) thread.join();
        }

        if (statistics) *statistics = measured;
        metrics.increment(metric_id::project_composition_file_count,
                          static_cast<std::uint64_t>(measured.configuration_files_loaded));
        metrics.increment(metric_id::project_composition_cache_hit_count,
                          static_cast<std::uint64_t>(cache_hits));
        metrics.set(metric_id::project_composition_max_parallel_workers,
                    static_cast<std::int64_t>(measured.max_active_workers));
        ...
        if (first_failure)
        {
            const auto* descriptor = diagnostic_registry.find(first_failure->id);
            try_emit(diagnostics, descriptor ? *descriptor : diagnostics::project_composition_failed,
                     operation, first_failure->offset);
            return {first_failure->code};
        }

        enum class visit_state : std::uint8_t { unseen, visiting, done };
        struct visit_frame
        {
            std::filesystem::path path;
        ...
        stack.push_back({root_path, &root->second->configuration, 0});

        while (!stack.empty())
        {
            auto& frame = stack.back();
            if (frame.next_item == frame.configuration->project.size())
            {
            ...
            const auto state_position =
                states.try_emplace(item.path, visit_state::unseen).first;
            if (state_position->second == visit_state::visiting)
            {
                try_emit(diagnostics, diagnostics::project_composition_cycle, operation);
                return {status_code::configuration_failed};
            }
            if (state_position->second == visit_state::done) continue;
    ...
    { try_emit(diagnostics, diagnostics::project_composition_failed, operation); return {status_code::configuration_failed}; }
